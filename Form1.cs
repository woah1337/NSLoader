...
{
	public partial class Form1: Form { [DllImport("InjectLib.dll", CallingConvention = CallingConvention.Cdecl)]
		...
		private void nsButton1_Click(object sender, EventArgs e) {
			...
			if (nsComboBox1.SelectedIndex == 0) //if the first item is selected
			{
				...
				if (nsComboBox1.SelectedIndex == 1) // if the second item is selected
				{
					...
					else MessageBox.Show("Couldn't Select Cheat", "Error"); // if there is an error, the user will get a messagebox that says couldnt select cheat
				}
			}
